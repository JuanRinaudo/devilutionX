...
void InitMonster(int i, int rd, int mtype, int x, int y)
{
	...
	if (gnDifficulty == DIFF_NIGHTMARE) {
		...
		if (gbIsHellfire)
			monster[i]._mmaxhp += (gbIsMultiplayer ? 100 : 50) << HPMANASHIFT;
		else
			monster[i]._mmaxhp += 64;
		monster[i]._mhitpoints = monster[i]._mmaxhp;
		monster[i].mLevel += 15;
		...
		if (gbIsHellfire)
			monster[i]._mmaxhp += (gbIsMultiplayer ? 200 : 100) << HPMANASHIFT;
		else
...
void PlaceUniqueMonst(int uniqindex, int miniontype, int bosspacksize)
{
	...
	if (gnDifficulty == DIFF_NIGHTMARE) {
		...
		if (gbIsHellfire)
			Monst->_mmaxhp += (gbIsMultiplayer ? 100 : 50) << HPMANASHIFT;
		else
			Monst->_mmaxhp += 64;
		Monst->mLevel += 15;
		Monst->_mhitpoints = Monst->_mmaxhp;
		...
		if (gbIsHellfire)
			Monst->_mmaxhp += (gbIsMultiplayer ? 200 : 100) << HPMANASHIFT;
		else
			Monst->_mmaxhp += 192;
		Monst->mLevel += 30;
		Monst->_mhitpoints = Monst->_mmaxhp;
	...
	if (Monst->_mFlags & MFLAG_BERSERK || !(Monst->_mFlags & MFLAG_GOLEM)) {
		for (pnum = 0; pnum < MAX_PLRS; pnum++) {
			if (!plr[pnum].plractive || currlevel != plr[pnum].plrlevel || plr[pnum]._pLvlChanging
			    || ((plr[pnum]._pHitPoints >> HPMANASHIFT) == 0) && gbIsMultiplayer)
				continue;
